...
// bit masks
#define BMASK      (COL9 | COL10 | COL11 | COL12 | COL13)
#define FMASK      (COL7 | COL8)
#define RROWMASKC  (RROW2)
#define RROWMASKD  (RROW1 | RROW3)
#define RROWMASKE  (RROW4)
...
static matrix_row_t read_cols(uint8_t row) {
    if (row < 6) {
        ...
        // Need to read 3 ports, and merge them nicely.
        return ~(
            ((PINF & FMASK) >> 6) // COL01
          | ((PINB & 0x02)  << 1) // COL2
          | ((PINB & 0x08)  >> 1) // COL3
